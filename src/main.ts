...
const main = async () => {
    ...
    const autoLogService = ServiceManager.createIntervalService(
        ...
        () => {
            const strCurrentTime = (new Date()).toLocaleString('VN');
            storage.messages.push(`[autoLogService] ${strCurrentTime} Tự động thêm dòng này mỗi 5s`);

            //? Log ra để biết hàm này đang được gọi
            console.log('[autoLogService]', strCurrentTime, 'Tự động log mỗi 5s đang dược gọi...');
    ...
    const autoSendMailService = ServiceManager.createTimeService(
        ...
        // Mã quốc gia
        'VN' // Mỗi quốc gia sẽ có múi giờ khác nhau. Ví dụ 12h trưa ở VN sẽ là khoảng 0h sáng tại Mỹ.
        // Server có thể đặt tại bất kỳ vị trí nào trên thế giới nên để set thời gian thì cần một múi giờ tại một quốc gia làm mốc.
        // Danh sách mã quốc gia có thể tham khảo: https://www.w3schools.com/tags/ref_country_codes.asp
    );
